...
db.serialize(function() {
  db.each("SELECT course FROM keywordID WHERE keyword LIKE '%"+keyword+"%'",function(err,rows){ 
    if (rows==null){res.send("No Match keyword!");}
    ...
    if(exist==-1&&rows.course!=undefined){ //new added
      ...
      for(var i=0;i<course_output.length;i++){
        ...
        var param={
          ...
          'courseName':jsonQuery("courseNo[id="+course_output[i]+"].name",{data:data}).value,
          'point':count[i]
        }
        console.log(course_output[i]+","+jsonQuery("courseNo[id="+course_output[i]+"].name",{data:data}).value+","+count[i]);
        arr.push(param);
